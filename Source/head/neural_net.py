...
class FourLayerConvNet_fast(object):
    ...
    def loss(self, X, y=None):
        ...
        pool_param = {'pool_height': 2, 'pool_width': 2, 'stride': 2}
        pool_param_last = {'pool_height': 4, 'pool_width': 4, 'stride': 4}
        ...
        dmax_pool = max_pool_backward_fast(daffine, max_cache[1])             
        dX = relu_backward(dmax_pool, conv_relu_cache[1])
        dX, grads['W2'], grads['b2'] = conv_backward_fast(dX, conv_cache[1])

        dmax_pool = max_pool_backward_fast(daffine, max_cache[0])           
