...
import numpy as np
try:
    from head.utils.im2col_cython import col2im_cython, im2col_cython
    from head.utils.im2col_cython import col2im_6d_cython
except ImportError:
    print('run the following from the cs231n directory and try again:')
    print('python setup.py build_ext --inplace')
    print('You may also need to restart your iPython kernel')

from head.utils.im2col import *
...
def max_pool_forward_im2col(x, pool_param):
    ...
    # If H is an odd number, pad x at one side.
    #if (H - pool_height) % stride != 0:
    #    assert stride % 2 == 0
    #    x = np.pad(x, ((0, 0), (0, 0), (0, int(stride//2)), (0, int(stride//2))), mode='constant')
    ...
    # im2col is not defined, so I changed it to im2col_indices, but error occurs.
